...
public sealed class GoogleMapsClient :
	...
	[Obsolete($"Use {nameof(DistanceMatrixAsync)} instead. Will be removed in 2.1.0.")]
	...
	[Obsolete($"Use {nameof(ElevationAsync)} instead. Will be removed in 2.1.0.")]
	...
	[Obsolete($"Use {nameof(GeocodeAsync)} instead. Will be removed in 2.1.0.")]
	...
	[Obsolete($"Use {nameof(ReverseGeocodeAsync)} instead. Will be removed in 2.1.0.")]
	public async Task<GeocodeResponse> GetReverseGeocodeAsync(
		string location) => await GetGeocodeAsync(new GeocodeRequest {
			Location = location
		});

	[Obsolete($"Use {nameof(TimeZoneAsync)} instead. Will be removed in 2.1.0.")]
	...
	private async Task<string> GetResponseAsync(
		RequestBase request) {
		var endpoint = $"{request.Endpoint}&key={_key}";
		...
		try {
			...
			using var message = await _httpClient.PostAsync(endpoint, content);
